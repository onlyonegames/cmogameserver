...
public class MailDto {
    ...
    public Mail ToEntity() {
        return Mail.builder().fromId(fromId).toId(toId).title(title).content(content).gettingItems(gettingItems).gettingItemCounts(gettingItemCounts).sendDate(sendDate).expireDate(expireDate).mailType(mailType).build();
    }

    public void InitFromDbData(Mail dbData) {
        this.id = dbData.getId();
        this.fromId = dbData.getFromId();

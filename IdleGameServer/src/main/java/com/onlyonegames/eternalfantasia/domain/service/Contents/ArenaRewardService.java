...
public class ArenaRewardService {
    ...
    public Map<String, Object> GetArenaReward(Long userId, Map<String, Object> map) {
        ...
        if(previousArenaRanking == null) {
            errorLoggingService.SetErrorLog(userId, ResponseErrorCode.NOT_FIND_DATA.getIntegerValue(), "Fail! -> Cause: PreviousArenaRanking not find.", this.getClass().getSimpleName(), Thread.currentThread().getStackTrace()[1].getMethodName(), Thread.currentThread().getStackTrace()[1].getLineNumber(), IS_DIRECT_WRIGHDB);
        ...
        previousArenaRanking.ReceiveReward();
        return map;
    }
}
